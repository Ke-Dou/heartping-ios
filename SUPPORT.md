# HeartPing Support 💕

Welcome to HeartPing support! We're here to help you stay connected with your loved ones.

## 📧 Contact Support
For any questions or issues, please email: **koipapa8@gmail.com**

## 🤔 Frequently Asked Questions

### How do I add friends?
1. Open HeartPing
2. Tap "Friends" 
3. Tap "Add Friends"
4. Search by username and send a friend request

### How do I send love?
1. Select a friend from your list
2. Tap the big heart button ❤️
3. Your friend receives an instant notification!

### I'm not receiving notifications
1. Check iOS Settings → Notifications → HeartPing
2. Ensure notifications are enabled
3. Try restarting the app

### How do I manage my profile?
1. Tap "Profile" in the app
2. Edit your display name and username
3. Upload a profile picture
4. Manage your account settings

### How do I delete my account?
1. Go to Profile → Danger Zone
2. Tap "Delete Account"
3. Confirm with your password
4. All your data will be permanently deleted

## 🔒 Privacy & Security
- We only collect essential information (email, display name)
- Your data is securely stored with Firebase
- We never sell or share your personal information
- See our Privacy Policy for full details

## 📱 App Information
- **Current Version**: 1.0
- **Platform**: iOS 14.0+
- **Developer**: [Your Name]

## 🐛 Report a Bug
Found a bug? Please email us with:
- Your device model and iOS version
- Steps to reproduce the issue
- Screenshots if helpful

---
Made with ❤️ for staying connected
